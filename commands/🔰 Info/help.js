...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    if (args[0]) {
      ...
      let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("905348589897797642").setLabel("Support").setURL("https://discord.gg/kyGRVPh4xs")
      let menuOptions = [{
          label: "Overview",
          value: "Overview",
          emoji: "905348589897797642",
...
<a:adev:847720420140515338> Support [**Premier Crafty**](https://discord.gg/kyGRVPh4xs)`)
    ...
    function allotherembeds_eachcategory(filterdisabled = false) {
      ...
      var embed0 = new MessageEmbed()
        ...
        .addField(`<a:ndarrow:907631365384720384> **Server Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField(`<a:ndarrow:907631365384720384> **Bot Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "bot").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed0)
      ...
      var embed3 = new MessageEmbed()
        ...
        .addField("<a:ndarrow:907631365384720384> **Song Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("song")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<a:ndarrow:907631365384720384> **Bot Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed3)
      ...
      var embed5 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<a:ndarrow:907631365384720384> **Bot Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      ...
      var embed9 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<a:ndarrow:907631365384720384> **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("info")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<a:ndarrow:907631365384720384> **Adjust the Bot**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed9)
